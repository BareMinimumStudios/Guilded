...
import keno.guildedparties.api.client.custom.GPSurfaces;
import keno.guildedparties.api.client.screens.ActionConfirmScreen;
import keno.guildedparties.api.client.screens.own_guild.management.GuildManagementMenu;
...
public class OwnGuildMenu extends BaseUIModelScreen<FlowLayout> {
    ...
    public OwnGuildMenu(Member member, Map<String, Rank> players,
        ...
        super(FlowLayout.class, DataSource.asset(GuildedParties.modLoc("own_guild_ui")));
        this.member = member;
        ...
        Identifier textureId = this.customTextures ? GuildedParties.modLoc(member.getGuildKey()
                .strip().toLowerCase().replace(" ", "_")): Identifier.of("minecraft", "");
    ...
    public FlowLayout getGuildDescriptionElement(UIModel model) {
        ...
        layout.childById(ButtonComponent.class, "leave-button").onPress(button
                ...
                new StrParamServerPacket(this.member.getGuildKey(), "guild", StrParamServerPacket.LEAVE_GUILD))));
        ...
        layout.childById(ButtonComponent.class, "invite-button").onPress(button
                ...
                NoParamServerPacket.GET_INVITABLE_PLAYERS)));

        layout.childById(ButtonComponent.class, "settings-button")
                .active(this.member.getRank().isCoLeader())
                .onPress(button
                        -> GPNetworking.GP_CHANNEL.clientHandle().send(new StrParamServerPacket(this.member.getGuildKey(), "guild",
                        StrParamServerPacket.GET_GUILD_SETTINGS)));
